...
when the edge passes the existing short face rule. The first two upper floors can receive painted
stone toned surrounds that share the exact generated window layout. The stoop and surrounds are
illustrative. They are not observed entrances or facade geometry. Every other building keeps the
earlier rendering rule. The current Rust context classifier uses 257,183 named
